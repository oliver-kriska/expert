defmodule Engine.DocumentSymbol do
  @moduledoc false
  ...
  defp walker({:defmodule, meta, [name | children]}, _mod) do
    ...
    %DocumentSymbol{
      ...
      children:
        List.flatten(for(child <- children, sym = walker(child, name), sym != nil, do: sym)),
      range: %Range{
  ...
  defp walker({:describe, meta, [name | children]}, mod) do
    ...
    %DocumentSymbol{
      ...
      children:
        List.flatten(for(child <- children, sym = walker(child, mod), sym != nil, do: sym)),
      range: %Range{
        start: %Position{line: meta[:line] - 1, character: meta[:column] - 1},
        end: %Position{line: meta[:end][:line] - 1, character: meta[:end][:column] - 1}
      },
  ...
  defp walker({type, meta, [name | _children]}, _)
       when type in [:def, :defp, :defmacro, :defmacro] do
    %DocumentSymbol{
  ...
  defp elixir_kind_to_lsp_kind(kind)
       when kind in [:def, :defp, :defmacro, :defmacrop, :test, :describe],
