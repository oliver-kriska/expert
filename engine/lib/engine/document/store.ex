defmodule Engine.Document.Store do
  @moduledoc """
  ...
  defmodule State do
    ...
    @spec fetch(t, Engine.uri()) :: {:ok, Document.t(), t} | {:error, :not_open}
    ...
    @spec fetch(t, Engine.uri(), Store.derivation_key()) ::
    ...
    @spec save(t, Engine.uri()) :: {:ok, t} | {:error, :not_open}
    ...
    @spec open(t, Engine.uri(), String.t(), pos_integer(), String.t()) ::
    ...
    @spec open?(t, Engine.uri()) :: boolean
    def open?(%__MODULE__{} = store, uri) do
      Map.has_key?(store.open, uri)
    end

    @spec close(t, Engine.uri()) :: {:ok, t} | {:error, :not_open}
    ...
    @spec get_and_update(t, Engine.uri(), Store.updater()) ::
    ...
    @spec update(t, Engine.uri(), Store.updater()) :: {:ok, t} | {:error, any()}
    ...
    @spec open_temporarily(t, Engine.uri() | Path.t(), timeout()) ::
    ...
    @spec extend_timeout(t, Engine.uri(), timeout()) :: t
  ...
  @spec close(Engine.uri()) :: :ok | {:error, :not_open}
  def close(uri) do
    GenServer.call(name(), {:close, uri})
  end

  @spec get_and_update(Engine.uri(), updater) :: {:ok, Document.t()} | {:error, any()}
  def get_and_update(uri, update_fn) do
    GenServer.call(name(), {:get_and_update, uri, update_fn})
  end

  @spec update(Engine.uri(), updater) :: :ok | {:error, any()}
